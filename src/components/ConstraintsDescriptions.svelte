<script lang="ts">
  ...
  $: visTotalHeight = Math.max(
    visViewHeight,
    constraint.explaining_features.length *
      (2 * raincloudMinHeight + margin.top + margin.bottom)
  );

  type Key = 'satisfies' | 'unsatisfies';
  const keys = ['satisfies', 'unsatisfies'] as const;

  $: categoricalFeatures = constraint.explaining_features.filter(
    (f) => $feature_info[f].kind === 'categorical'
  );

  $: color = scaleOrdinal<Key, string>()
    .domain(keys)
    .range(categoricalColors.medium);

  // y-scale for faceting by feature
  $: fy = scaleBand<string>()
    .domain(constraint.explaining_features)
    .range([0, visTotalHeight]);

  $: I = range($num_instances);
  $: filteredI = range($num_instances);

  // map from satisfies/unsatisfies to number of instances
  type SectionToCount = InternMap<Key, number>;
  // the first number represents the categorical feature value (as a number)
  type PointData = [number, SectionToCount];

  $: stackGenerator = stack<PointData, Key>()
  ...
  $: x = Object.fromEntries(
    constraint.explaining_features.map((f) => {
  ...
  $: distributions = constraint.explaining_features.map((f) => {
    const info = $feature_info[f];
    ...
    if (info.kind === 'categorical') {
      ...
      const aggregate = rollup(
        ...
        (i) => (constraint.satisfies.includes(i) ? 'satisfies' : 'unsatisfies')
      );
      ...
      return {
        feature: f,
        kind: 'quantitative',
        data: Array.from(aggregate, ([section, raincloud]) => ({
          section,
          raincloud,
        })),
      };
  ...
  $: if (group && constraint.explaining_features && visWidth > 0) {
    setupBrush(brush);
  }
  ...
  function drawAllRaincloudPlots(
    ...
    clusterY: ScaleBand<'satisfies' | 'unsatisfies'>,
    color: ScaleOrdinal<'satisfies' | 'unsatisfies', string>,
    featureData: FeatureData[],
    width: number,
    height: number
  ) {
    ...
    featureData.forEach(({ feature, kind, data }) => {
      ...
      data.forEach(({ section, raincloud }) => {
        const clusterYCoord = clusterY(section) ?? 0;
        ...
        {#each distributions as d}
          <g transform="translate(0,{fy(d.feature)})">
            {#if d.kind === 'categorical'}
              <g>
                {#each d.data as series}
                  <g
                    fill={series.key === 'satisfies' ||
                    series.key === 'unsatisfies'
                      ? color(series.key)
                      : 'red'}
                  >
            ...
            <g class="x-axis" id="axis-{d.feature}">
              <XAxis
                ...
                label={$feature_info[d.feature].display}
              />
            </g>
          </g>
        {/each}
